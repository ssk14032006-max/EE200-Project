  ...
  """, unsafe_allow_html=True)
    ...
    if fig2 is not None:
        ...
        <p style="color: {MUTED}; font-size: 15px; line-height: 1.6;">
            ...
            <i>{os.path.splitext(result["song"])[0]}</i> reconstructed from the database. Each dot is a stored hash anchor. The highlighted window is exactly where the query clip sits inside the full song.
        </p>
  """, unsafe_allow_html=True)
